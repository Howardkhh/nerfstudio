...
"""
Nerfacto augmented with depth supervision and SparseNeRF's distill continuity.
"""
...
from nerfstudio.model_components.losses import DepthLossType, depth_loss, depth_ranking_loss, distill_continuity_loss
from nerfstudio.models.nerfacto import NerfactoModel, NerfactoModelConfig
from nerfstudio.utils import colormaps


@dataclass
class SparseNerfactoModelConfig(NerfactoModelConfig):
    ...
    depth_loss_type: DepthLossType = DepthLossType.SPARSENERF_RANKING
    """Depth loss type."""


class SparseNerfactoModel(NerfactoModel):
    """Depth loss augmented nerfacto model and SparseNeRF's distill continuity.

    Args:
        config: Nerfacto configuration to instantiate model
    """

    config: SparseNerfactoModelConfig
        ...
        if self.training:
            ...
            if self.config.depth_loss_type in (DepthLossType.DS_NERF, DepthLossType.URF):
                ...
                metrics_dict["depth_ranking"] = depth_ranking_loss(
                    outputs["expected_depth"], batch["depth_image"].to(self.device), group_size=8
                )
            else:
                raise NotImplementedError(f"Unknown depth loss type {self.config.depth_loss_type}")
            
            metrics_dict["distill_continuity"] = distill_continuity_loss(outputs["expected_depth"])
        return metrics_dict
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.training:
            ...
            loss_dict["distill_continuity_loss"] = self.config.distill_continuity_mult * metrics_dict["distill_continuity"]
        return loss_dict
